...
const MenuItem = ({ item, onChange }) => {
  ...
  return (
    <Group id={item.instanceId} x={item.x} y={item.y} {...getItemHandlers()} {...item}>
      {/* Background Card */}
      {/* <Rect
      ...
      {item.showTitle && (
        <Text text={item.foodItem.name || 'MenuItem'} x={180} y={10} {...item.titleStyle} />
      )}
      ...
      {item.showPrice && (
        <Text text={item.foodItem.price || 0} x={180} y={70} {...item.priceStyle} />
      )}
    </Group>
  );
};
